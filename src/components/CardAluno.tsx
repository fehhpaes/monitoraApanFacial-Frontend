...
export default function CardAluno({ aluno, onEdit, onDelete, onGenerateQR, onDeleteQR }: CardAlunoProps) {
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition">
      ...
      <div className="p-4">
        ...
        <div className="space-y-2">
          ...
          {onGenerateQR && (
            <button
              onClick={() => onGenerateQR(aluno._id)}
              disabled={aluno.qrCodeGerado}
              className="w-full bg-green-600 hover:bg-green-700 disabled:bg-gray-400 text-white font-semibold py-2 px-3 rounded-lg transition flex items-center justify-center gap-2"
            >
              <QrCode size={16} />
              {aluno.qrCodeGerado ? 'QR Gerado' : 'Gerar QR Code'}
            </button>
          )}

          {onDeleteQR && aluno.qrCodeGerado && (
            <button
              onClick={() => onDeleteQR(aluno._id)}
              className="w-full bg-orange-600 hover:bg-orange-700 text-white font-semibold py-2 px-3 rounded-lg transition flex items-center justify-center gap-2 text-sm"
            >
              <Trash2 size={14} />
              Deletar QR Code
            </button>
